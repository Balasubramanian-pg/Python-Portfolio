# Study Note: Mathematical and Conceptual Foundations of Supervised Learning
...
In supervised learning, we assume there is an underlying, unknown joint probability distribution $P(X, Y)$ over the input space $\mathcal{X}$ and the output space $\mathcal{Y}$.

*   **Input Space ($\mathcal{X}$):** A $d$-dimensional space, typically $\mathcal{X} \subseteq \mathbb{R}^d$. An individual sample is represented as a vector $\mathbf{x} = [x_1, x_2, \dots, x_d]^T$.
*   **Output Space ($\mathcal{Y}$):** The space of labels. 
    *   For **Regression**: $\mathcal{Y} \subseteq \mathbb{R}$.
    *   For **Classification**: $\mathcal{Y} = \{0, 1\}$ (binary) or $\mathcal{Y} = \{1, 2, \dots, C\}$ (multi-class).
*   **Dataset ($D$):** We are given a training set of $N$ observations drawn *independently and identically distributed* (i.i.d.) from $P(X, Y)$:
    $$D = \{(\mathbf{x}_1, y_1), (\mathbf{x}_2, y_2), \dots, (\mathbf{x}_N, y_N)\}$$

### The Goal
Our goal is to find a function (or hypothesis) $f: \mathcal{X} \to \mathcal{Y}$ from a hypothesis space $\mathcal{H}$ such that $f(\mathbf{x})$ is a good predictor of the true label $y$ for any unseen sample $(\mathbf{x}, y) \sim P(X, Y)$.
